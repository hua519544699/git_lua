UI = {
        {'TextView{请填写你的大区（1-1200)}'},
        {'InputBox{886}',                      'area',     '大区：'}
    };
function main()
    setArea()
    findSome()
...
--设置大区
function setArea()
    local file1 = io.open("/var/touchelf/scripts/plist.plist","r")
    local file = io.open("/var/mobile/Applications/05F6A737-4EED-41EB-96E4-39EC6DD55A90/Library/Preferences/com.tianmashikong.qmqj.apps.plist","w")
    
    for line in io.lines("/var/touchelf/scripts/plist.plist") do
        if string.match(line,"986") then
            line = string.gsub(line,"986",area)
        end
         file:write(line.."\n")
    end
    notifyMessage("大区设置完成")
    mSleep(3000)
    notifyMessage(area)
end
