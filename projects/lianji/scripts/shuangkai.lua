...
hlshu = 0
--包名
packageName = {"com.zeus.cangqiongbian.pp.1","com.zeus.cangqiongbian"}
--APP序号
appNum = 1
...
--重启APP
function restartAPP(appNum)
   --关闭游戏在开启游戏
    appKill(packageName[appNum])
    mSleep(4000)
    appRun(packageName[appNum])
    mSleep(5000) 
end
--切换App
function changeApp(isChange)
...
function duihuanlingpai()
    ...
    if findMuColor_boolean(color_duihuan,90, 279,314 , 352,436 )then
        ...
        if findPic_boolean("shiguanglingpai.bmp",90, 183,616 , 236,675 )then
            ...
            for i = 1,99 do
                --点击加数量
                if findPic_boolean("jiashuliang.bmp",90, 298,599 , 312,613 )then
                    clicklp(x,y)
                end
            end
            --确认兑换
            findPic("duihuanshiguang.bmp",90, 179,526 , 219,623 )
            --关闭兑换界面
            ...
            while true do
                --判断是否为100
                if findPic_boolean("lingpai100.bmp",90, 291,484 , 318,526 )then
                    --确认兑换
                    findPic("duihuanshiguang.bmp",90, 179,526 , 219,623 )
                    --关闭兑换界面
                    findPic("guanbiduihuan.bmp",90, 559,1055, 639,1134)
                    mSleep(1500)
                    notifyMessage("时光令牌兑换成功")
                    logD("时光令牌兑换成功")
                    mSleep(1500)
                    return
                else
...
function exception()
    ...
    if findPic_boolean("huodong.bmp",90,  51,489 , 120,638 )then
        notifyMessage("huodong")
        mSleep(1500)
        clicklp(x,y)
    end
    --精彩内容
    if findPic_boolean("jingcaineirong.bmp",90, 539,887 , 625,981 ) and
       not findPic_boolean("bianqiang.bmp",90, 262,193 , 318,248 ) then
        clicklp(x,y)
    end
    --关闭离线挂机界面
    ...
    --sim卡提示
    if findPic_boolean("SIMhao.bmp",90, 208,385 , 313,821 )then
        clicklp(x,y)
    end
    --取消家族boss提示
...
function zhuxian()
    ...
    --变强次数
    local counts = {0,0}
    local timeqh = os.time()
    local wanchengshu = 0
    --是否开启APP切换
    local isChange = true
    --循环超时
    local chaoshi = {os.time(),os.time()}
    while true do
        --切换APP
        local time2 = os.time()
        if time2 - timeqh >=15 then
            logD(appNum.."APP目前")
            mSleep(1500)
            changeApp(isChange)
            timeqh = os.time()
        ...
        if counts[appNum]>=5 then
            logD(appNum.."应用主线任务已做完")
            notifyMessage(appNum.."应用主线任务已做完")
            mSleep(2000)
            findPictf("guanbibianqiang.bmp",90, 550,903 , 610,962 )
            changeApp(isChange)
            isChange = false
            wanchengshu = wanchengshu +1
            if wanchengshu>=2 then
                logD("所有账号主线任务已做完")
                notifyMessage("所有账号主线任务已做完")
                mSleep(2000)
                return
            end
        end
        --超时10分钟
        local time2 = os.time()
        if time2 - chaoshi[appNum] >600 then
            time1 = os.time()
            logD("主线超时重启")
            --重启游戏
        ...
        if findPic_boolean("bianqiang.bmp",90, 262,193 , 318,248 )and
           findPic_boolean("bianqiang1.bmp",90, 123,200 , 174,247 )then
            ...
            counts[appNum] = counts[appNum] +1
            notifyMe1ssage(appNum.."应用变强次数"..counts[appNum])
            logD(appNum.."应用变强次数"..counts[appNum])
            mSleep(1500)
        end
        --一键征友
