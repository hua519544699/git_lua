...
--等级礼包
color_dengjilibao = { 0xD62084, 33, 4, 0xE7DB7B, 30, 42, 0xAD7100, 77, 74, 0xCE3839, 75, 68, 0xC62C29, 78, 76, 0xD63839, 79, 65, 0xEFC3BD, 75, 65, 0xEFC7C6, 76, 71, 0xC62C31 }
--满斗气
...
--纳戒宝库
color_najiebaoku = { 0xFFEBC6, 14, 22, 0xFFFBD6, 26, -8, 0xF7EBC6, 31, -1, 0xD69E6B, 44, 36, 0xCE3431, 48, 32, 0xF74D4A, 38, 30, 0xF7E3DE, 36, 36, 0xDECBC6, 42, 34, 0xC62021, 45, 42, 0xD62429 }
--萧家族地
...
function clearBag()
    ...
    if findPic_boolean("beibao.bmp",90,   4,531 ,  57,594 ) then
        ...
        while hh < 4 do
            ...
            if findOneColor_bool(0xFFFFB5,85,428-lh*102,560 +hh*101,525-lh*102,660+hh*101) or 
               findOneColor_bool(0xE751E7,85,428-lh*102,560 +hh*101,525-lh*102,660+hh*101) or 
               findOneColor_bool(0xC6DBEF,85,428-lh*102,560 +hh*101,525-lh*102,660+hh*101) or 
               findOneColor_bool(0x29D7BD,85,428-lh*102,560 +hh*101,525-lh*102,660+hh*101) then
                click(x,y)
                ----[[
                --如果是药品,讨伐令,钱币,装备,铸灵碎片,悬赏令                      不处理的东西
                if findPic_boolean("hongyao1.bmp",90, 440,14  , 513,92  ) or
                   ...
                   findPic_boolean("xuanshangling",90, 443,19  , 511,90  )or
...
--买药
function maiyao()
    if findPicLostColor_boolean("yaoguan.bmp",90, 194,378 , 270,627 ,0xFF00FF) then
        --关闭npc对话
    ...
    ---[[
        ...
        --click( 321,148 )
        ------------------------------------------------------------------------------------------------------------------------
        if findPic_boolean("yaopinganniu.bmp",90, 271,8   , 376,327 ) then
            click(x,y)
            --买红
            --click( 435,200 )
        ...
        --点击药罐
        findPicLostColor("yaoguan.bmp",90, 194,378 , 270,627 ,0xFF00FF)
        --确定补给
        findPic("quedingbuji.bmp",90, 208,430 , 245,522 )
        --确定出售白装
        findPic("baizhuangqueding.bmp",90, 208,430 , 245,522)
        --关闭背包
...
function zhencha()
    ...
    while true do
        --清理背包  31分钟
        timebb = os.time()
        if timebb%1860<60 and
        not findPic_boolean("duihua.bmp",90,   1,1091,  33,1121) and
        not findPic_boolean("tiaoguo.bmp",90, 579,0   , 638,108 ) then
            --关闭
            guanbi()
            --关闭npc对话
            findPictf("guanbiNPC.bmp",90, 575,277 , 638,327 )
        ...
        --精彩内容1
        findPic("jingcaineirong1.bmp",90, 565,921 , 621,976 )
        --关闭离线挂机界面
        ...
        if time4%180 <7 then
            ...
            if findMuColor_boolean(color_duihuan,90, 279,314 , 352,436 ) then
                if not findPic_boolean("kaishizhencha.bmp",90,   3,819 ,  98,994 ) and
        ...
        --150环完成判断
        --[[if findMuColor_boolean(color_renwu,90, 288,0, 408,94) then
            if not findMuColor_boolean(color_zhencha,90, 248,10  , 368,188 ) then
                notifyMessage("150换侦查任务已完成")
                mSleep(1500)
                logD("150换侦查任务已完成")
                return
            end
...
function luopan()
    ...
    while true do
        --没过2分钟进行一次副本匹配
        local time3 = os.time()
        if time3 % 120 < 10 then
            notifyMessage("防止超时匹配,再次进行匹配")
            mSleep(1500)
            --关闭弹窗
            guanbi()
            --后台到前台
            ...
            findPic("kunnan.bmp",90,  62,585 , 180,714 )
            --点击自动匹配
            findPicTime("zidongpipei.bmp",90,  24,575 ,  90,714 )
            --关闭副本进入界面
            findPic("guanbifuben.bmp",90, 578,1073, 638,1131)
        end
        --进入游戏
        ...
        --确认副本进入界面
        findPic("querenjinru.bmp",90, 202,423 , 252,529 )
        --关闭弹窗
        --guanbi()
        --纳戒
...
function qiyu()
    ...
    --有没有使用次数
    if findMuColor_boolean(color_mianfei,90,  50,459 ,  79,540) then
        --摩擦神灯
...
function najie()
    ...
    findPic("richang.bmp",90, 460,938 , 560,1043)
    --纳戒宝库
    if findMuColor_boolean(color_najiebaoku,90, 382,596 , 470,686 ) then
        click(x,y)
        --抽奖
...
function xuanshang()
    ...
    time1 = os.time()
    while true do
        if findPic_boolean("xuanshangjieshou.bmp",90,  24,220 ,  99,371 ) or 
            findPic_boolean("qianwang.bmp",90,  31,246 ,  89,358 ) then
            --悬赏次数已用完
            notifyMessage("悬赏次数已用完")
            logD("悬赏次数已用完")
            --关闭悬赏界面
            findPic("guanbixuanshang.bmp",90, 578,1066, 638,1134)
            mSleep(1500)
            return
        end
        --超时8判断
        time2 = os.time()
        if time2 - time1 >480 then
            --重置时间
            time1 = os.time()
        ...
        --无悬赏字样悬赏完成
        if not findMuColor_boolean(color_xsrw,90, 230,65  , 384,284 ) then
            notifyMessage("悬赏结束")
            mSleep(1500)
            --挂机
            findMuColor(color_guaji,90, 308,1040, 390,1131)
    ...
    if findMuColor_boolean(color_mandouqi,90,6,678 ,30,754 ) then
        ...
        for i=1,10 do
            if findPic_boolean("tupo.bmp",90,  10,1012,  85,1121) then
                ...
                mSleep(5000)
                --跳过动画
                findPic("tiaoguo.bmp",90, 601,65  , 633,99  )
                mSleep(5000)
            end
            --点击修炼
            if findPic_boolean("xiulian.bmp",90,  16,1014,  87,1125) then
                click(x,y)
                mSleep(5000)
                --跳过动画
                findPic("tiaoguo.bmp",90, 601,65  , 633,99  )
        ...
        mSleep(5000)
        --斗气不足
        findPic("douqibuzu.bmp",90, 219,526 , 252,601 )
        mSleep(5000)
        --关闭斗气界面
...
function lingjiang()
    ...
    findPic("lingjiang1.bmp",90, 541,938 , 630,1035)
    --等级礼包
    if findMuColor_boolean(color_dengjilibao,90, 325,241 , 501,406 )then
        click(x,y)
        --领取
    ...
    --关闭7日签到界面
    findPic("guanbi7ri.bmp",90, 582,905 , 636,959 )
...
function zhuxian()
    ...
    while true do
        ...
        qiantai()
        --释放技能
        if findMuColor_boolean(color_jineng,90, 206,323 , 239,505 ) then
            --释放技能
            clicktf(89,1052)
            --释放技能
            clicktf(89,1052)
        ...
        --关闭配件
        findPic("guanbipeijian.bmp",90, 227,509 , 291,628 )
        --跳过动画
        --findMuColortf(color_tiaoguo,90, 593,0   , 638,106 )
        findPictf("tiaoguo.bmp",90, 601,65  , 633,99  )
        --跳过对话
        findPictf("duihua.bmp",90,   1,1091,  33,1121)
        --跳过对话
        findPictf("duihua.bmp",90,   1,1091,  33,1121)
        --领取红包
        findPictf("hongbao.bmp",90, 477,749 , 526,798 )
        --提交任务
        findPicTime("tijiaorenwu.bmp",90,28,8, 126,323 )
        --接受任务
        findPicTime("jieshourenwu.bmp",90,  34,9   , 127,325 )
        ...
        --离开副本
        if findMuColor_boolean(color_likaifuben,90,  18,957 ,  70,1098) then
            mSleep(8000)
            clicktf(x,y)
        end
        --进入游戏
        ...
        --出现变强界面退出循环
        if findPic_boolean("bianqiang.bmp",90, 563,558 , 596,600 ) then
            --关闭变强界面
            clicktf( 585,934 )
            logD("主线任务已做完")
        ...
        --斗魂修炼
        findMuColortf(color_douhunxiulian,90,   5,888 ,  91,1096)
        --点击修炼
        findPictf("xiulian.bmp",90,  16,1014,  87,1125)
        --点击修炼
        findPictf("xiulian.bmp",90,  16,1014,  87,1125)
        --斗气不足
...
function create()
    ...
    while true do
        ...
        --切换账号
        findPictf("qiehuanzhanghao.bmp",90, 290,658 , 348,782 )
        --快速注册
...
function guanbi()
    ...
    --退出签到
    findPictf("tuichuqiandao.bmp",90, 557,841 , 612,904 )
