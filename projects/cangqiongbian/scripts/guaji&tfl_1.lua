    ...
    if findPic_boolean("fasongyoujian.bmp",90, 115,826 , 192,986 ) then
        ...
        while counts <3 do
            while wz < 8 do
                ...
                if findOneColor_bool(0x5A2C21,85,15,218+wz*78,90,296+wz*78) or
                   findOneColor_bool(0x39144A,85,15,218+wz*78,90,296+wz*78) or
                   findOneColor_bool(0x102052,85,15,218+wz*78,90,296+wz*78) or
                   findOneColor_bool(0x004942,85,15,218+wz*78,90,296+wz*78) then
                    click(x,y)
                    if findPic_boolean("shenmiqianbi.bmp",90, 441,16  , 516,96  ) then
                        --关闭物品信息
                        findPic("guanbiwuping.bmp",90, 520,283 , 571,336 )
                    end
                    --寄出
            ...
            ----[[
            if findOneColor_bool(0x5A2C21,85, 114,533 , 201,628 ) or
               findOneColor_bool(0x39144A,85, 114,533 , 201,628 ) or
               findOneColor_bool(0x102052,85, 114,533 , 201,628 ) or
               findOneColor_bool(0x004942,85, 114,533 , 201,628 ) then
                notifyMessage("物品已放满")
                mSleep(1500)
                logD("物品已放满")
                break
...
---[[    
    ...
    while true do
        ...
        --跳过对话
        findPictf("duihua1.bmp",90,   1,1085,  36,1124)
        --断网重试
        ...
        --断线重启
        x,y = findImageInRegionFuzzy(PIC_PATH.."gantanhao.bmp",90, 166,464 , 200,504 )
        if x~=-1 and y ~= -1 then
...
function lianxinFB()
    ...
    --判断是否有次数
    x,y = findImageInRegionFuzzy(PIC_PATH.."jinruyanyang.bmp",90,   9,744 , 116,926 )
    --判断副本是否有次数
    if x~=-1 and y ~=-1 then
    ...
    while true do
        ...
        --是否在副本里面
        x,y = findImageInRegionFuzzy(PIC_PATH.."guanbiyanyang.bmp",90, 579,1069, 639,1133)
        if x~=-1 and y ~=-1 then
            return
        end
        --副本超时30分钟  重启1800
...
---[[
    ...
    --点击药店商人
    findPic("NPC.bmp",90, 326,992 , 393,1061)
    --click( 205,541 )
    --点按钮买药
    --click( 321,148 )
    ------------------------------------------------------------------------------------------------------------------------
...
function guanbi()
    ...
    qiantai()
    --连接资源
    x,y = findImageInRegionFuzzy(PIC_PATH.."lianjieziyuan.bmp",90, 223,367 , 403,756 )
    if x~=-1 and y~=-1 then
        logD("连接资源卡死。重启")
        --关闭游戏在开启游戏
        appKill("com.zeus.cangqiongbian")
    ...
    --连接资源
    x,y = findImageInRegionFuzzy(PIC_PATH.."lianjieziyuan_1.bmp",90, 223,367 , 403,756 )
    if x~=-1 and y~=-1 then
        logD("连接资源1卡死。重启")
        --关闭游戏在开启游戏
        appKill("com.zeus.cangqiongbian")
    ...
    --登陆界面卡死，无账号区号信息
        x,y = findImageInRegionFuzzy(PIC_PATH.."noaccount.bmp",90, 179,378 , 336,762 )
        if x~=-1 and y~=-1 then
            logD("无账号区号信息卡死。重启")
            --关闭游戏在开启游戏
            appKill("com.zeus.cangqiongbian")
